...
| key | 说明 |
| --- | --- |
| `READ_NUM` | 可选，单个窗口的阅读次数（每次约 30 秒），默认 18 次 ≈ 10 分钟 |
| `BOOK_ID` | 可选，阅读的书籍 ID，默认《三体全集》（`ce032b305a9bc1ce0b0dd2a`），换其它书需自行测试时长是否计入 |

### 3. 运行

- 定时运行：workflow **每小时整点**运行一次，每次约 10 分钟（18 次），全天 24 小时接力（见 `.github/workflows/weread.yml`，可自行修改 cron）。
...
4. **风控观察**：当前默认约 432 次/天（每小时 18 次 × 24）。部署后前几天请观察推送结果与 App 内实际时长；若出现频繁掉 `synckey`、强制退登等风控迹象，请调低 `READ_NUM`。
